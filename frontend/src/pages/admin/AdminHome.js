...
function AdminHome() {
  ...
  const handleSearch = (query) => {
    ...
    fetchUsers(`${baseURL}/api/users/admin/users/?search=${query}`);
  };

  useEffect(() => {
    fetchUsers(baseURL + "/api/users/admin/users/");
  }, []);

  useEffect(() => {
    fetchUsers(`${baseURL}/api/users/admin/users/?search=${searchQuery}`);
  }, [searchQuery]);
